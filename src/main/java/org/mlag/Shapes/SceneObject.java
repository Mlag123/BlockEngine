...
public abstract class SceneObject {
    ...
    public void rotate(float angleDegrees, float ax, float ay, float az) {
        ...
        modelMatrix.identity().translate(position)  // вернуть объект на место
    ...
    public void rotateX(float angleDegrees) {
        ...
        this.angle = angleDegrees;
        modelMatrix.rotate((float) Math.toRadians(angleDegrees), 1f, 0f, 0f);
    }

    public void rotateY(float angleDegrees) {
        // Вращение вокруг оси Y → (0, 1, 0)
        this.angle = angleDegrees;
        modelMatrix.rotate((float) Math.toRadians(angleDegrees), 0f, 1f, 0f);
    }

    public void rotateZ(float angleDegrees) {
        // Вращение вокруг оси Z → (0, 0, 1)
        this.angle = angleDegrees;
        modelMatrix.rotate((float) Math.toRadians(angleDegrees), 0f, 0f, 1f);
    }
...
/*    protected void initColliderFromMesh(float[] vertices) {
    ...
    public void moveObjectFromCollider(Vector3f vec, AABB aabb) {
        Vector3f oldPos = new Vector3f(this.position);

        this.translate(vec);


        boolean hasCollision = AABB.intersects(this.collider, aabb);
        System.out.println("Collision after move: " + hasCollision);
        ...
        // Восстанавливаем модельную матрицу
    }
}
